...
export const filterItalicContent = (tokens: AST.Inlines[]): AST.Italic['value'] => {
...
export const filterStrikeContent = (tokens: AST.Inlines[]): AST.Strike['value'] =>
	tokens.filter((token) => token.type !== 'STRIKE' && token.type !== 'IMAGE' && token.type !== 'COLOR' && token.type !== 'INLINE_KATEX');
