...
/**
 * Get full emoji character (including complex sequences)
 */
const getEmojiChar = (text: string, index: number): { char: string; length: number } => {
...
/**
 * Get a simple emoji (no modifiers)
 */
const getSimpleEmoji = (text: string, index: number): { char: string; length: number } => {
	const char = text[index];
	const nextChar = text[index + 1];
 ...
 * Detect URLs
 */
const detectURL = (text: string, startIndex: number): { url: string; length: number } | null => {
	...
	if (atIndex !== -1 && !/^(https?|ftp|ssh|[a-zA-Z]+):\/\//.test(remaining)) {
		...
		if (
			(spaceBeforeAt === -1 || spaceBeforeAt > atIndex) &&
			(spaceAfterAt === -1 || spaceAfterAt > dotAfterAt) &&
			dotAfterAt > 0 &&
			dotAfterAt < afterAt.length - 1
		) {
 ...
 */
const detectEmail = (text: string, startIndex: number): { email: string; length: number } | null => {
...
/**
 * Check if a consecutive emoticon sequence should be treated as plain text
 */
...
/**
 * Parse bold markup in the text.
 */
const parseBoldMarkup = (text: string, i: number, options?: Options): { tokens: AST.Inlines[]; nextIndex: number } | null => {
	const char = text[i];
...
/**
 * Parse italic markup in the text.
 */
const parseItalicMarkup = (text: string, i: number, options?: Options): { tokens: AST.Inlines[]; nextIndex: number } | null => {
	const char = text[i];
	const nextChar = text[i + 1];
	...
	const atStartWordBoundary = i === 0 || /[\s\n\r\t\(\)\[\]{}.,;:!?*~`-]/.test(prevChar) || (prevChar === '_' && followingChar === '_');
	...
	if (singleEnd !== -1 && singleEnd > i + 1) {
		...
		if (atSingleEndWordBoundary) {
			...
			if (singleContent.trim().length > 0) {
...
/**
 * Parse strikethrough markup
 *
 * @example
 * '~~Hello~~' -> [strike]Hello[/strike]
 */
const parseStrikeMarkup = (text: string, i: number, options?: Options): { tokens: AST.Inlines[]; nextIndex: number } | null => {
	const char = text[i];
	const nextChar = text[i + 1];
...
/**
 * Parse inline content with markup.
 *
 * @example
 * parseInlineContent('Hello *world*') === [plain('Hello '), bold([plain('world')])]
 */
	...
	while (i < n) {
			...
			if (/https?$/.test(beforeColon) || /^:\/\//.test(afterColon)) {
				...
				if (atWordBoundary && !afterQuote) {
					...
					if (endIndex !== -1 && endIndex > i + 1) {
						...
						if (shortCode && /^[a-zA-Z0-9_+-]+$/.test(shortCode) && shortCode.length >= 2 && !/^\d+$/.test(shortCode)) {
								...
								if (labelText) {
									...
									const filteredTokens = parsedTokens.filter(
										(token): token is AST.Plain | AST.Bold | AST.Italic | AST.Strike =>
											token.type === 'PLAIN_TEXT' || token.type === 'BOLD' || token.type === 'ITALIC' || token.type === 'STRIKE',
									);
		...
		while (tempI < n) {
			...
			if (currentChar === '@') {
				...
				const emailBefore =
					/[a-zA-Z0-9._'-]/.test(lastCharBefore) ||
					(lastCharBefore && lastCharBefore.charCodeAt(0) > 127 && !/[\s\n\r\t\(\)\[\]{}.,;:!?@]/.test(lastCharBefore));
...
/**
 * Detect emails in plain text and split into tokens
 */
...
export const parse = (input: string, options?: Options): AST.Root => {
	...
	for (const line of lines) {
		...
		const isOnlyEmoji =
			...
			inlineContent.every((token) => {
				return token.type === 'EMOJI' || (token.type === 'PLAIN_TEXT' && /^\s*$/.test(token.value));
			});
	...
	while (i < lines.length) {
		...
		if (
			(starter === '-' || starter === '*') &&
			RE_UNORDERED_BULLET.test(line) &&
			!RE_UNORDERED_BULLET_EMPTY.test(line) &&
			!RE_UNORDERED_STAR_INLINE.test(line)
		) {
		...
		if (line.trim() === '') {
			...
			for (let k = 0; canFastPath && k < ln; k++) {
				...
				if (((code & 0x80) === 0 && TRIGGER_ASCII[code] === 1) || ch === '\\' || ch === '+') {
					canFastPath = false;
					break;
				}
				...
				if (code > 127) {
					...
					if (probe) {
						...
						if (emojiProbe.length > 1) {
							k += emojiProbe.length - 1;
						}
					}
					continue;
				}
